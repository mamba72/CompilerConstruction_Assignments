...
-- these are the same functions, just for DFA and NFAs
...
nfa2dfa nfa = DFA {
  ...
  dfa_final = let final in
    final [] = return [] -- if there are no states, return an empty list
    final (q:qs) = if disjunction [q] then return [q]
                      else return final qs
    --final state:states = 
    in final,
 -- dfa_delta = let delta qs c = qs in delta }
  dfa_delta = let delta in
    delta [] c = return []
    delta (q:qs) c = do
      next <- delta q c
      run delta qs next
